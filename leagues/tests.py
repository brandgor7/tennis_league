...
class SeasonPlayerDetailViewTest(TestCase):
    """The player detail URL now redirects to team detail."""

    def setUp(self):
        self.season = make_season()
        self.player = make_player('alice', first='Alice', last='Smith')
        self.team = enroll(self.season, self.player, tier=1)
        self.url = reverse('leagues:player_detail', kwargs={
            'slug': self.season.slug,
            'username': self.player.username,
        })

    def test_redirects_to_team_detail(self):
        response = self.client.get(self.url)
        self.assertRedirects(
            response,
            reverse('leagues:team_detail', args=[self.season.slug, self.team.slug]),
...
class TeamDetailViewTest(TestCase):
    ...
    def _url(self, season=None, team=None):
        ...
        return reverse('leagues:team_detail', args=[s.slug, t.slug])
    ...
    def test_404_for_unknown_season(self):
        url = reverse('leagues:team_detail', args=['no-such-season', self.team1.slug])
        self.assertEqual(self.client.get(url).status_code, 404)

    def test_404_for_team_not_in_season(self):
        other_season = make_season(name='Other Season', year=2024)
        url = reverse('leagues:team_detail', args=[other_season.slug, self.team1.slug])
        self.assertEqual(self.client.get(url).status_code, 404)
    ...
    def test_404_for_wrong_slug(self):
        url = reverse('leagues:team_detail', args=[self.season.slug, 'no-such-slug'])
        self.assertEqual(self.client.get(url).status_code, 404)


# ─── Team slug tests ──────────────────────────────────────────────────────────

class TeamSlugTest(TestCase):
    def setUp(self):
        self.season = Season.objects.create(name='Slug Season', year=2025, use_team_name=False)
        self.alice = User.objects.create_user(username='slug_alice', first_name='Alice', last_name='Smith')
        self.bob = User.objects.create_user(username='slug_bob', first_name='Bob', last_name='Jones')

    def _make_team(self, *members, name=''):
        team = Team.objects.create(season=self.season, name=name)
        for m in members:
            team.members.add(m)
        team.set_slug()
        return team

    def test_slug_set_from_last_name_for_singles(self):
        team = self._make_team(self.alice)
        self.assertEqual(team.slug, 'smith')

    def test_slug_set_from_last_names_for_doubles(self):
        team = self._make_team(self.alice, self.bob)
        self.assertIn('smith', team.slug)
        self.assertIn('jones', team.slug)

    def test_slug_ordered_alphabetically_by_last_name(self):
        team = self._make_team(self.alice, self.bob)
        self.assertEqual(team.slug, 'jones-smith')

    def test_slug_uses_team_name_when_use_team_name_true(self):
        self.season.use_team_name = True
        self.season.save()
        team = self._make_team(self.alice, self.bob, name='Aces')
        self.assertEqual(team.slug, 'aces')

    def test_slug_falls_back_to_members_when_name_absent_and_use_team_name_true(self):
        self.season.use_team_name = True
        self.season.save()
        team = self._make_team(self.alice)
        self.assertEqual(team.slug, 'smith')
    ...
    def test_slug_collision_gets_numeric_suffix(self):
        self._make_team(self.alice)
        alice2 = User.objects.create_user(username='slug_alice2', first_name='Ann', last_name='Smith')
        team2 = self._make_team(alice2)
        self.assertEqual(team2.slug, 'smith-1')

    def test_slug_uses_username_when_no_last_name(self):
        noname = User.objects.create_user(username='noname_slug')
        team = self._make_team(noname)
        self.assertEqual(team.slug, 'noname_slug')

    def test_save_sets_fallback_slug_before_members_added(self):
        team = Team.objects.create(season=self.season)
        self.assertTrue(team.slug.startswith('team-'))

    def test_set_slug_updates_slug_in_db(self):
        team = self._make_team(self.alice)
        team.refresh_from_db()
        self.assertEqual(team.slug, 'smith')

    def test_team_detail_url_uses_slug(self):
        team = self._make_team(self.alice)
        url = reverse('leagues:team_detail', args=[self.season.slug, team.slug])
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)


# ─── TeamDetailView schedule display filter tests ─────────────────────────────

class TeamDetailDisplayFilterTest(TestCase):
    """schedule_display_mode filtering on the team detail upcoming list."""

    def setUp(self):
        self.season = make_season()
        self.player = make_player('tdf_p1', first='Alpha', last='Aaa')
        self.opponent = make_player('tdf_p2', first='Beta', last='Bbb')
        self.team = enroll(self.season, self.player, tier=1)
        self.opp_team = enroll(self.season, self.opponent, tier=1)
        self.url = reverse('leagues:team_detail', args=[self.season.slug, self.team.slug])
        self.today = datetime.date.today()

    def _match(self, **kwargs):
        defaults = dict(
            season=self.season, player1=self.player, player2=self.opponent,
            team1=self.team, team2=self.opp_team,
            tier=1, status=Match.STATUS_SCHEDULED,
        )
        defaults.update(kwargs)
